...
| **Echo** — Voice search | Vosk + [Chroma](https://www.trychroma.com/) + TTS | Transcribes your speech, runs a semantic search over a local knowledge base, and speaks the best match back |
...
```bash
pip install pyttsx3 vosk sounddevice chromadb
```
...
### Echo — voice search ([`echo()`](wireless_livestttts_stt_tts.py#L108))

A conversational loop that turns speech into a query against a local Chroma vector database and speaks the matched document back: it listens, transcribes a phrase, runs a semantic search, prints the matches, speaks the top result, and prompts you for the next one.

- **Semantic search** — the transcribed phrase is passed to [`SearchService.search()`](service/SearchService.py), which queries a [Chroma](https://www.trychroma.com/) collection (`knowledge_base`) and returns the top match. Chroma returns `documents` as a list-of-lists (one inner list per query), so the top document is unwrapped before it's spoken; if nothing matches, a fallback message is spoken instead.
- **Knowledge base** — documents, metadata, and IDs are loaded from [`resources/data.json`](resources/data.json) into Chroma on startup (see [`AppConfig`](config/AppConfig.py)). The database is persisted under `my_chroma_db/`.
...
```
python-stt-tts-wireless/
├── wireless_livestttts_stt_tts.py   # TTS + STT + echo (voice search) CLI
├── config/                 # AppConfig: Chroma client + loads resources/data.json
├── service/                # SearchService: semantic search over the Chroma collection
├── resources/
│   └── data.json           # knowledge-base documents, metadatas, ids
├── requirements.txt        # Python dependencies
├── .gitignore              # ignores model/, my_chroma_db/, __pycache__/, venvs, etc.
├── model/                  # Vosk model(s) — git-ignored, downloaded separately
├── my_chroma_db/           # persisted Chroma database — git-ignored, created at runtime
└── README.md
```

> Both `model/` and `my_chroma_db/` are git-ignored: the Vosk model is large and downloaded separately, and the Chroma database is regenerated from [`resources/data.json`](resources/data.json) on each run.
